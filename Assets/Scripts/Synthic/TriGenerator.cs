...
{
    [BurstCompile]
    public class TriGenerator : SynthProvider
    {
            ...
            double phase, int sampleRate, float amplitude, float frequency)
        {
            ...
            for (int sample = 0; sample < buffer.Length; sample++)
            {
                ...
                float s = (float)(amplitude/frequency * 2) * ((frequency * 2) - Mathf.Abs(sample % (4 * frequency) - (frequency * 2)) );
                s *= amplitude;
                buffer[sample] = new StereoData(s);
